...
import { type DetailedStats, type RatedFilm } from "../../lib/stats";
...
/**
 * Vizualizarea completă a statisticilor detaliate. Reutilizată de pagina proprie
 * /stats și de pagina de admin care inspectează statisticile altui user.
 * Titlul și subtitlul vin din exterior; restul etichetelor din namespace-ul Stats.
 */
export default async function StatsView({
  ...
  return (
    <main className="p-4 sm:p-8 max-w-5xl mx-auto flex-1 w-full">
      {backHref && backLabel && (
        <Link href={backHref} className="text-xs text-yellow-500 hover:text-yellow-400 font-medium">← {backLabel}</Link>
      )}
      <h2 className={`text-2xl font-semibold mb-2 border-l-4 border-yellow-500 pl-3 ${backHref ? "mt-2" : ""}`}>{title}</h2>
      <p className="text-zinc-500 mb-8 pl-4">{subtitle}</p>
