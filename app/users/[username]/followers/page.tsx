...
export default async function FollowersPage({
  ...
  const users: UserListItem[] = rows.map((r) => r.follower);
  ...
  return (
    <main className="p-8 max-w-3xl mx-auto flex-1 w-full">
      ...
      <h2 className="text-2xl font-semibold mt-2 mb-6 border-l-4 border-yellow-500 pl-3">{t("followersTitle")}</h2>
      <UserList users={users} followingSet={followingSet} viewerUserId={viewerUserId ?? null} emptyLabel={t("followersEmpty")} />
    </main>
  );
}
